...
def test_station_config_has_valid_entries(station_config):
    for station in station_config:
        assert 'name' in station and isinstance(station['name'], str), f'{station["name"]} is not a string'
        assert 'lat' in station and isinstance(station['lat'], float), f'{station["lat"]} is not a float'
        assert 'lon' in station and isinstance(station['lon'], float), f'{station["lon"]} is not a float'
...
@pytest.mark.parametrize("timeshift", [3, 12, 24])
def test_fetch_available_base_time_with_timeshift_of(ecmwf, timeshift):
    base_time_no_shift = ecmwf._fetch_available_base_time(fallback=True, timeshift=0)
    base_time = ecmwf._fetch_available_base_time(fallback=True, timeshift=timeshift)
    # check that the time is shifted by the correct amount
    base_time_dt = datetime.strptime(base_time, ecmwf._time_format)
    base_time_no_shift_dt = datetime.strptime(base_time_no_shift, ecmwf._time_format)
    assert (base_time_no_shift_dt - base_time_dt).total_seconds() == timeshift * 3600
...
def test_upgrade_base_time_if_api_request_fails(ecmwf):
    ...
    ecmwf._base_time = ecmwf._fetch_available_base_time(fallback=True, timeshift=12)
    base_time_shifted = ecmwf._base_time
    with patch.object(EcmwfApi, '_fetch_available_base_time', side_effect=ValueError):
        ecmwf.upgrade_basetime()
